...
.PHONY : install clean ipetopdf mathmacros depend zhmakeindex config preamble pgftopdf plantumltopdf chirun plastex
...
plastex :
	lyx -f -e xetex LearningMath.lyx
	python3 tex-for-plasTeX.py
chirun :
	. ~/chirun_env/bin/activate; chirun -v --config config-for-chirun.yml; deactivate